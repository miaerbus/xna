...
namespace Artificial.XNATutorial.Physics
{
    public class ParticleParticleCollisions : Part
    {
        ...
        void DetectCollision(float dt, Item item1, Item item2)
        {
            ...
            if (distance < r)
            {
                CustomCollider cc1 = item1.As<CustomCollider>();
                CustomCollider cc2 = item2.As<CustomCollider>();
                ...
                if (cc1 != null)
                {
                    apply &= !cc1.OverrideCollisionWithType.Contains(item2.GetType());
                    apply &= !cc1.OverrideCollisionWithItem.Contains(item2);
                }
                if (cc2 != null)
                {
                    apply &= !cc2.OverrideCollisionWithType.Contains(item1.GetType());
                    apply &= !cc2.OverrideCollisionWithItem.Contains(item1);
                }
                ...
                if (cc1 != null && cc1.CollisionMethod != null) cc1.CollisionMethod(dt, item2, pImpact);
                if (cc2 != null && cc2.CollisionMethod != null) cc2.CollisionMethod(dt, item1, pImpact);
            }
