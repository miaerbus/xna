...
namespace Artificial.XNATutorial.Physics
{
    ...
    public class PhysicsSimulator
    {
        ...
        public void Simulate(GameTime gameTime)
        {
            ...
            for (int i = 0; i < indices.Count; i++)
            {
                object sceneObject = scene[indices[i]];
                Spring itemSpring = sceneObject as Spring;
                PositionConstraint itemConstraint = sceneObject as PositionConstraint;
                Item item = sceneObject as Item;
                Movable itemMovable = null;
                ForceUser itemForceUser = null;
                Rotatable itemRotatable = null;
                TorqueUser itemTorqueUser = null;
                ParticleCollider itemParticleCollider = null;
                RigidCollider itemRigidCollider = null;
                if (item != null)
                {
                    itemMovable = item.As<Movable>();
                    itemForceUser = item.As<ForceUser>();
                    itemRotatable = item.As<Rotatable>();
                    itemTorqueUser = item.As<TorqueUser>();
                    itemParticleCollider = item.As<ParticleCollider>();
                    itemRigidCollider = item.As<RigidCollider>();
                }
                ...
                if (itemParticleCollider != null || itemRigidCollider != null)
                {
                    ...
                    for (int j = 0; j < filteredItems.Count; j++)
                    {
                        ...
                        {
                            // Get affected item
                            object sceneObject2 = (node.Value as CustomBoundingBoxABTItem).PositionPart;
                            Item item2 = sceneObject2 as Item;
                            if (item2 != item)
                            {
                                IParticleCollider itemParticleCollider2 = item2.As<ParticleCollider>();
                                IRigidCollider itemRigidCollider2 = item2.As<RigidCollider>();
