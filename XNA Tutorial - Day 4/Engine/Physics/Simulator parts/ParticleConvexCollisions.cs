    ...
    public class ParticleConvexCollisions : Part
    {
        ...
        void DetectCollision(float dt, Item item1, Item item2)
        {
            ...
            if (distance < r)
            {
                ...
                if (collides)
                {
                    for (int k = 0; k < c.Convex.Points.Count; k++)
                    {
                        // Project particle onto convex point
                        float min;
                        Vector3 position;
                        axis = c.Convex.Points[k] - position1;
                        axis.Normalize();
                        c.Convex.MinimumOnAxis(axis, position1, out min, out position);
                        float dx = min - p.ParticleRadius;
                        if (dx > minX)
                        {
                            minPlane.Normal = -axis;
                            minPlane.D = c.Convex.Points[k].Length();
                            minX = dx;
                        }
                        if (dx > 0)
                        {
