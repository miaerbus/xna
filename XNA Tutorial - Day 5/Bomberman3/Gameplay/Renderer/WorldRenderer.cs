...
namespace Artificial.XNATutorial.Bomberman
{
    class WorldRenderer : DrawableGameComponent
    {
        ...
        Vector3 cameraShake;
        float shakeSpeed = Gameplay.GridSize * 2;
        float shakePower;
        ...
        public override void Initialize()
        {
            ...
            alien.NormalMatrix = Matrix.CreateRotationX(MathHelper.PiOver2) * Matrix.CreateRotationY(-MathHelper.PiOver2) * Matrix.CreateScale(0.1f) * Matrix.CreateTranslation(0, 2.5f - Gameplay.GridSize * 0.5f, 0);
            ...
            pumpkin.NormalMatrix = Matrix.CreateRotationX(MathHelper.PiOver2) * Matrix.CreateRotationY(-MathHelper.PiOver2) * Matrix.CreateScale(0.1f) * Matrix.CreateTranslation(0, 2.5f - Gameplay.GridSize * 0.5f, 0);
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            // Update animations
            List<int> indices = Gameplay.Level.Scene.GetItemIndices();
            for (int i = 0; i < indices.Count; i++)
            {
                Item sceneItem = Gameplay.Level.Scene[indices[i]];
                ...
                if (sceneItem.Is<Explosion>())
                {
                    ...
                    if (e.Direction == Vector3.Zero)
                    {
                        if (e.Frame == 1)
                        {
                            Vector3 p = e.Part<PPosition>().Position + Vector3.Up * Gameplay.GridSize * 0.5f;
                            for (int j = 0; j < 3; j++)
                            {
                                float distance = 0.1f;
                                Vector3 o = new Vector3();
                                o.X += ((float)Bomberman.Random.NextDouble() - 0.5f) * Gameplay.GridSize * distance;
                                o.Z += ((float)Bomberman.Random.NextDouble() - 0.5f) * Gameplay.GridSize * distance;
                                o.Y += ((float)Bomberman.Random.NextDouble() - 0.5f) * Gameplay.GridSize * distance;
                    ...
                    else
                    {
                        if (e.Frame == 1)
                        {
                            Vector3 p = e.Part<PPosition>().Position + Vector3.Up * Gameplay.GridSize * 0.5f - e.Direction * Gameplay.GridSize;
                            for (int j = 0; j < 6; j++)
                            {
                                p += j * e.Direction * Gameplay.GridSize * 0.1f;
                                float distance = 0.1f;
                                Vector3 o = new Vector3();
                                o.X += ((float)Bomberman.Random.NextDouble() - 0.5f) * Gameplay.GridSize * distance;
                                o.Z += ((float)Bomberman.Random.NextDouble() - 0.5f) * Gameplay.GridSize * distance;
        ...
        public override void Draw(GameTime gameTime)
        {
            ...
            // Camera shaking
            cameraTarget = Gameplay.Level.Camera.Target;
            Gameplay.Level.Camera.Target = cameraTarget + cameraShake * shakePower;
            ...
            staticModelsShader.View = Gameplay.Level.Camera.ViewMatrix;
            staticModelsShader.Projection = Gameplay.Level.Camera.ProjectionMatrix;
            ...
            animatedModelsShader.Parameters["View"].SetValue(Gameplay.Level.Camera.ViewMatrix);
            animatedModelsShader.Parameters["Projection"].SetValue(Gameplay.Level.Camera.ProjectionMatrix);
            for (int i = 0; i < indices.Count; i++)
            {
                Item sceneItem = Gameplay.Level.Scene[indices[i]];
                if (sceneItem.Is<Character>())
            ...
            Gameplay.Level.Camera.Target = cameraTarget;
