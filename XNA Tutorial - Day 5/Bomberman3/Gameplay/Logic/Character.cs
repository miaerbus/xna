...
namespace Artificial.XNATutorial.Bomberman
{
    public class Character : Item
    {
        ...
        public Character(PlayerIndex player, CharacterType type)
        {
            ...
            Require<PBoundingSphere>().BoundingSphere = new BoundingSphere(Vector3.Zero, Gameplay.GridSize * 0.5f);
            Require<ForceUser>().Mass = 1000;
            Require<ParticleCollider>().ParticleRadius = Gameplay.GridSize * 0.4f;
        ...
        void update(float dt)
        {
                ...
                Input.gamePadState[(int)player].Buttons.A == ButtonState.Pressed)
            {
                if (BombCount > 0 && collider.OverrideCollisionWithItem.Count==0)
                {
                    ...
                    r.X = ((int)(r.X / Gameplay.GridSize) + 0.5f) * Gameplay.GridSize;
                    r.Z = ((int)(r.Z / Gameplay.GridSize) + 0.5f) * Gameplay.GridSize;
                    r.Y = Gameplay.GridSize;
                    Bomb newBomb = new Bomb(r, this, Gameplay.BombFuseTime, ExplosionPower);
                    Gameplay.Level.AddBomb(newBomb);
                    BombCount--;
                }
            }
        }
