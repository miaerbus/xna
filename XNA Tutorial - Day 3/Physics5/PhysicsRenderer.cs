...
namespace Artificial.XNATutorial.Physics
{
    class PhysicsRenderer : GameComponent
    {
        ...
        Color boundingSphereColor = Color.Red;

        bool renderSeparatingAxis = false;
        Color axisCollideColor = Color.Lime;
        Color axisDontCollideColor = Color.Green;
        ...
        public void Render(Camera camera)
        {
            ...
            LinkedListNode<IABTItem> node, node2;
            ...
            for (int i = 0; i < visibleItems.Count; i++)
            {
                ...
                while (node != null)
                {
                    ...
                    IBoundingSphere itemBoundingSphere = item as IBoundingSphere;
                    ...
                    if (renderSeparatingAxis && (itemParticleCollider != null || itemRigidCollider != null))
                    {
                        ...
                        for (int j = 0; j < filteredItems.Count; j++)
                        {
                            ...
                            node2 = filteredItems[j].First;
                            while (node2 != null)
                            {
                                // Get affected item
                                object item2 = (node2.Value as CustomBoundingBoxABTItem).PositionPart;
                                if (item2 != item)
                                {
                                    ...
                                    if (itemRigidCollider != null && itemRigidCollider2 != null)
                                    {
                                        ...
                                        if (distance < r)
                                        {
                                            m1 = itemRigidCollider.Rotation * Matrix.CreateTranslation(itemRigidCollider.Position);
                                            Matrix m2 = itemRigidCollider2.Rotation * Matrix.CreateTranslation(itemRigidCollider2.Position);
                                            Matrix n = m1 * Matrix.Invert(m2);

                                            for (int k = 0; k < itemRigidCollider.ConvexPolyhedron.Planes.Count; k++)
                                            {
                                                // Project body onto normal
                                                Plane p = itemRigidCollider.ConvexPolyhedron.Planes[k];
                                                float min1, max1, min2, max2;
                                                itemRigidCollider.ConvexPolyhedron.ProjectToAxis(p.Normal, Vector3.Zero, out min1, out max1);
                                                Vector3 localAxis = Vector3.TransformNormal(p.Normal, n);
                                                Vector3 localZero = Vector3.Transform(Vector3.Zero, n);
                                                Vector3 pImpact;
                                                itemRigidCollider2.ConvexPolyhedron.ProjectToAxis(localAxis, localZero, out min2, out max2);
                                                //itemRigidCollider2.ConvexPolyhedron.MinimumOnAxis(localAxis, localZero, out min2, out pImpact);
                                                ...
                                                {
                                                    primitiveBatch.DrawLine(v1, v2, 1, axisCollideColor);
                                                }
                                                else
                                                {
                                                    primitiveBatch.DrawLine(v1, v2, 1, axisDontCollideColor);
                                                }
                                            }
                                        }
                                    }
